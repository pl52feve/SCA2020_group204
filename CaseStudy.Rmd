...
#andere Anzahl belegter Raumtypen als reservierte? -- Bene Raumtypen
...
################################Tests#################################################################
undefined_tests = rawdata[rawdata$meal == "Undefined",]
#summary(undefined_tests)

for (j in 1:8) {
  undefined_tests = scramble(undefined_tests)
  for(i in 1:(dim(undefined_tests)[1])){
    if(all_equal(undefined_tests[i,],undefined_tests[i+1,],ignore_col_order = TRUE, ignore_row_order = TRUE) == TRUE){
      undefined_tests = undefined_tests[-i,]
    }
...
#nur Resort Hotel
unique(undefined_tests$hotel)
#fast nur no Deposit
sum(undefined_tests$deposit_type=="No Deposit")
#hier nicht so viele Transient
#sum(undefined_tests$customer_type == "Transient")
#viele sehr ähnliche Buchungen

#sind nur zwei, werden rausgeschmissen
dim(rawdata[rawdata$market_segment == "Undefined",])[1]
clndata = clndata[!(clndata$market_segment == "Undefined"),]

#sind nur fünf Einträge, werden rausgeschmissen
dim(rawdata[rawdata$distribution_channel == "Undefined",])[1]
clndata = clndata[!(clndata$distribution_channel == "Undefined"),]


dim(rawdata[rawdata$country == "NULL",])[1]
undefined_tests_cntry = rawdata[rawdata$country == "NULL",]

for (j in 1:8) {
  undefined_tests_cntry = scramble(undefined_tests_cntry)
  for(i in 1:(dim(undefined_tests_cntry)[1])){
    if(all_equal(undefined_tests_cntry[i,],undefined_tests_cntry[i+1,],ignore_col_order = TRUE, ignore_row_order = TRUE) == TRUE){
      undefined_tests_cntry = undefined_tests_cntry[-i,]
    }
...
#fast alles Resort Hotel
sum(undefined_tests_cntry$hotel == "Resort Hotel")
#fast alle Transient
sum(undefined_tests_cntry$customer_type == "Transient")
#fast alle BB (aber generell haben das viele oder?)
sum(undefined_tests_cntry$meal == "BB")
...
for(i in (1:length(clndata_showup$stay_totaldays))){
  ...
  cmpltweeks = floor(total/7)
  rest = (total+checkin_wkday) %% 7
...
adultChild = guestNr - babies;
  ...
  for (j in idx:(idx+clndata_showup_resort$stay_totaldays[i]-1)){
    ...
    #an allen Tagen außer dem letzten wird ganz normal hochgezählt - macht das so Sinn?
    else if(j != idx+clndata_showup_resort$stay_totaldays[i]){
